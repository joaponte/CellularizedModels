...
class ModelSteppable(SteppableBasePy):
    ...
    def step(self, mcs):
        ...
        for cell in self.cell_list_by_type(self.DEAD):
            nbs_uninfected = 0
            for neighbor, common_surface_area in self.get_cell_neighbor_data_list(cell):
                if neighbor is not None and neighbor.type == self.UNINFECTED:
                    nbs_uninfected += 1

            if random.random() < pr_recovery * nbs_uninfected:
                cell.type = self.UNINFECTED

        # Test case: therapy -> no internalization, kill infected and virus-releasing cells
        if mcs == 100:
            self.sbml.ODEModel["beta"] = 0
