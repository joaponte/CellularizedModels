...
for r in range(1,replicates+1):
    ...
    I2[:, r - 1] = f['I2']
    D[:, r - 1] = f['D']
plt.plot(f['Time'],np.mean(U,1),color='blue',linewidth = 4.0,label='U')
plt.fill_between(f['Time'], np.amin(U, 1), np.amax(U, 1), facecolor='blue', interpolate=True, alpha=0.2)
plt.plot(f['Time'],np.mean(I1,1),color='orange',linewidth = 4.0,label='I1')
plt.fill_between(f['Time'], np.amin(I1, 1), np.amax(I1, 1), facecolor='orange', interpolate=True, alpha=0.2)
plt.plot(f['Time'],np.mean(I2,1),color='red',linewidth = 4.0,label='I2')
plt.fill_between(f['Time'], np.amin(I2, 1), np.amax(I2, 1), facecolor='red', interpolate=True, alpha=0.2)
plt.plot(f['Time'],np.mean(D,1),color='purple',linewidth = 4.0,label='D')
