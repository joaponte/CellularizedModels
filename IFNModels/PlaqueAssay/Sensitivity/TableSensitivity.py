...
np.savetxt('Tab.Baseline.GrowthRate.txt',heatmap_matrix.transpose(),delimiter = ',')
...
for j in range(len(parameters)):
    ...
    for i in range(len(p)):
        ...
        for r in range(1,replicates+1):
            f = np.genfromtxt('Data/FullModelCellular_%s_%.2f_%i.txt' % (parameters[j], p[i], r), skip_header=1,
                              delimiter=',', names=names, max_rows=t)
            iVeAUC[r-1] = np.sum(f['Ve'])
            iIFNeAUC[r-1] = np.sum(f['IFNe'])
            iVeMAX[r-1] = np.amax(f['Ve'])
            iIFNeMAX[r-1] = np.amax(f['IFNe'])
    ...
    IFNeAUCdown[j] = pIFNeAUC[0] / 1.0e4
    IFNeAUCdownPvalue[j] = pIFNeAUCPval[0]
    IFNeAUCup[j] = pIFNeAUC[1] / 1.0e4
    IFNeAUCupPvalue[j] = pIFNeAUCPval[1]
    IFNeMAXdown[j] = pIFNeMAX[0] / 1.0e3
    IFNeMAXdownPvalue[j] = pIFNeMAXPval[0]
    IFNeMAXup[j] = pIFNeMAX[1] / 1.0e3
...
np.savetxt('Tab.Baseline.VeAUC.txt',heatmap_matrix.transpose(),delimiter = ',')
sns.heatmap(heatmap_matrix.transpose(), yticklabels=parameters, xticklabels=Outputs, cmap = 'seismic',
            annot= True ,fmt='.3f',center=0.0)
plt.title('Ve AUC')
plt.tight_layout()
...
np.savetxt('Tab.Baseline.VeMAX.txt',heatmap_matrix.transpose(),delimiter = ',')
sns.heatmap(heatmap_matrix.transpose(), yticklabels=parameters, xticklabels=Outputs, cmap = 'seismic',
            annot= True ,fmt='.3f',center=0.0)
plt.title('Ve MAX')
plt.tight_layout()
...
BaselineIFNeAUC /= 1.0e4
BaselineIFNeAUCstd /= 1.0e4
Outputs = ['Baseline','stdev','Down','Diff_Down','p-value','Up','Diff_Up','p-value','Diff_Diff']
heatmap_matrix = np.zeros((9,len(parameters)))
...
np.savetxt('Tab.Baseline.IFNeAUC.txt',heatmap_matrix.transpose(),delimiter = ',')
sns.heatmap(heatmap_matrix.transpose(), yticklabels=parameters, xticklabels=Outputs, cmap = 'seismic',
            annot= True ,fmt='.3f',center=0.0)
plt.title('IFNe AUC')
plt.tight_layout()
...
BaselineIFNeMAX /= 1.0e3
BaselineIFNeMAXstd /= 1.0e3
Outputs = ['Baseline','stdev','Down','Diff_Down','p-value','Up','Diff_Up','p-value','Diff_Diff']
heatmap_matrix = np.zeros((9,len(parameters)))
...
np.savetxt('Tab.Baseline.IFNeMAX.txt',heatmap_matrix.transpose(),delimiter = ',')
sns.heatmap(heatmap_matrix.transpose(), yticklabels=parameters, xticklabels=Outputs, cmap = 'seismic',
            annot= True ,fmt='.3f',center=0.0)
plt.title('IFNe MAX')
plt.tight_layout()
