...
for r in range(1,21):
    ...
    plt.plot(T, f['CC3DIFNe_Scalar'], color='orange', linewidth=2.0, alpha=0.15)
plt.plot(ODEf['Time'][::2], ODEf['ODEIFNe'][::2],'.',color='orange',markersize=6,label='ODE')
plt.plot(T,np.mean(IFNe,1),color='orange',linewidth = 3.0,label='ABM_Field')
plt.plot(T,np.mean(IFNe_Scalar,1),color='chocolate',linewidth = 3.0,label='ABM_Scalar')
